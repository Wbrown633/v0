...
if __name__ == "__main__":
    ...
    input(
        "Connect serial cable to the PC port on the WASTE servo control board (#1) and hit ENTER."
    )

    print("SETTING WASTE ADR! ", p._set_addr(WASTE_ADDR))

    input(
        "Connect serial cable to the PC port on the LYSATE servo control board (#2) and hit ENTER."
    )

    print("SETTING LYSATE ADR! ", p._set_addr(LYSATE_ADDR))

    input("Setup serial cables as they will be used inproduction then hit ENTER to confirm.")
